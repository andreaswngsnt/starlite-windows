...
#Starlite User Interface
#Last Updated: 03/07/2024 at 7:45 PM
...
class CameraFeed(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        while True:
            ...
            cv2.resizeWindow("Received Frame", 500, 400)
...
class Manual_Control(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        control_toggle_key = 'off'

        client_socket.send(f"{control_toggle_key}".encode('utf-8'))

        def keyboard_listener():
            try:
                
                print("Manual control activated...")
                print("Press 'q' to disable robot.")
                ...
                while True:
                    ...
                    if (event.event_type == keyboard.KEY_DOWN) & ((key == 'w') | (key == 's') |(key == 'a') | (key == 'd') | (key == 'q')):
                        if key not in keys_pressed:
                            client_socket.send(f"{key}".encode('utf-8'))
                            continue
                    elif (event.event_type == keyboard.KEY_UP) & (key != 'q') & ((key == 'w') | (key == 's') | (key == 'a') | (key == 'd')):
                ...
                print("Delivery robot control deactivating...")
                client_socket.send(f"{control_toggle_key}".encode('utf-8'))
                print("Delivery robot control offline.")

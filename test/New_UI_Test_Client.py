...
class MenuBar(tk.Menu):
    def __init__(self, parent):
        ...
        menu_pricing.add_command(label="Map Location", command=lambda: parent.show_frame(PageOne))
        menu_pricing.add_command(label="Live Camera Feed", command=lambda: parent.show_frame(PageTwo))
                ...
                while True:
                    ...
                    if event.event_type == keyboard.KEY_DOWN:
                        if key not in keys_pressed:
                            client_socket.send(f"Key holding: {key}".encode('utf-8'))
                            keys_pressed.add(key)
                    elif event.event_type == keyboard.KEY_UP:
                        client_socket.send(f"Key released: {key}".encode('utf-8'))
